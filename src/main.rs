...
//fucked: Learn 2 error handle instead of just providing a empty ./token lmao
	...
	let mut tokenInput = File::open(tokenPath).unwrap();
	let mut tokenFileAsStr = String::new();
	tokenInput.read_to_string(&mut tokenFileAsStr);
//end of fucked

	
    let stored_token = Some(tokenFileAsStr.to_string());
    ...
    tokio::spawn(async move {
        ...
        while let Some(token) = new_tokens.next().await {
			...
			//fucked: Handle error b4 unwrapping
			let mut output = File::create(tokenPath).unwrap();
